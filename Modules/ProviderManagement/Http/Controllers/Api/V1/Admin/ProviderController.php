    ...
    public function index(Request $request): JsonResponse
{
    ...
    // Ensure logo_full_path is included in each provider
    $providers->getCollection()->transform(function ($provider) {
        $provider->logo_full_path = $provider->logo_full_path; // Accessor call
        return $provider;
    });

    $topCards = [
        'total_providers' => $this->provider->ofApproval(1)->count(),
        'total_onboarding_requests' => $this->provider->ofApproval(2)->count(),
        'total_active_providers' => $this->provider->ofApproval(1)->ofStatus(1)->count(),
        'total_inactive_providers' => $this->provider->ofApproval(1)->ofStatus(0)->count(),
    ];
