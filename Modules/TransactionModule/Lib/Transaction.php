...
if (!function_exists('completeBookingTransactionForDigitalPayment')) {
    function completeBookingTransactionForDigitalPayment($booking): void
    {
        ...
        DB::transaction(function () use ($booking, $admin_user_id, $provider_user_id, $admin_commission, $provider_earning, $promotional_cost_by_admin, $promotional_cost_by_provider) {
            ...
            // Calculate net payable to provider (deduct existing payable)
            $provider_existing_payable = $provider_account->account_payable;
            ...
            if ($net_provider_earning < 0) {
                ...
                // Provider's payable reduced but not fully cleared
                $remaining_payable = abs($provider_earning - $provider_existing_payable);
            } else {
                $remaining_payable = 0;
            }

            // 1. Admin Pending Transaction (unchanged)
            $account = Account::where('user_id', $admin_user_id)->first();
            $account->balance_pending -= $booking['total_booking_amount'];
            $account->save();
            ...
            // 2. Provider Receivable (NET of existing payable)
            $provider_account->account_receivable += $net_provider_earning;
            // Clear or reduce provider's payable
            $provider_account->account_payable = $remaining_payable;
            ...
            Transaction::create([
                ...
                'debit' => 0,
                'credit' => $net_provider_earning,
                'balance' => $provider_account->account_receivable,
                'from_user_id' => $admin_user_id,
                'to_user_id' => $provider_user_id,
                'from_user_account' => null,
            ...
            // 4. Admin Payable (NET of existing provider payable)
            $account = Account::where('user_id', $admin_user_id)->first();
            $account->account_payable += $net_provider_earning;
            $account->save();
            ...
            // 6. Expense transactions (unchanged)
            $account = Account::where('user_id', $admin_user_id)->first();
    ...
    function completeBookingTransactionForPartialCas($booking): void
    {
        ...
        DB::transaction(function () use ($booking, $admin_user_id, $provider_user_id, $wallet_paid, $cash_due, $admin_commission, $provider_earning, $provider_wallet_credit, $admin_commission_from_wallet, $wallet_amount_to_admin) {
            ...
            if ($cash_due > $provider_earning) {
                // Case 1: Cash > Earning - Provider owes excess
                $excess_cash = $cash_due - $provider_earning;

                // First, use receivable to offset excess
                ...
                // Remaining excess becomes payable
                if ($excess_cash > 0) {
                    $payable_creation = $excess_cash;
                    $providerAccount->account_payable += $payable_creation;
                }

                $provider_wallet_amount = 0;
            } else {
                // Case 2: Cash <= Earning - Provider gets wallet credit
                $provider_wallet_amount = $provider_wallet_credit;

                // Deduct existing payable from wallet credit
                if ($providerAccount->account_payable > 0 && $provider_wallet_amount > 0) {
                    $payable_deduction = min($provider_wallet_amount, $providerAccount->account_payable);
                    $provider_wallet_amount -= $payable_deduction;
                    $providerAccount->account_payable -= $payable_deduction;
                }

                // Add remaining to receivable
            ...
            if ($cash_due > 0) {
                // Calculate cash to provider (subtract receivable deduction and payable creation)
                $cash_to_provider = $cash_due - $receivable_deduction - $payable_creation;
...
if (!function_exists('referralEarningTransactionDuringRegistration')) {
    function referralEarningTransactionDuringRegistration($user, $amount)
    {
        ...
        // Registration referral has no booking reference, so keep this transaction booking_id as NULL.
...
if (!function_exists('referralEarningTransactionAfterBookingComplete')) {
    function referralEarningTransactionAfterBookingComplete($user, $amount, $bookingId = null)
    {
...
if (!function_exists('referralEarningTransactionAfterBookingCompleteFirst')) {
    ...
    {
        // $bookingID = $booking->parent_booking_id ?? $booking->id;
